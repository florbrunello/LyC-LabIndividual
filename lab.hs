...
instance DomSem Int where
  ...
  sem (Div e1 e2) σ =
    let d = sem e2 σ
    in if d == 0 then 0 else sem e1 σ `div` d
...
instance DomSem Ω where
  ...
  sem (Local v e c) σ =
    let result = sem c (update σ v (sem e σ))
        restore σf = update σf v (σ v)
    in case result of
        Normal σ'' -> Normal (restore σ'')
        Abort  σ'' -> Abort  (restore σ'')
  ...
  sem (While b c) σ = fix f σ
    where f :: (Σ -> Ω) -> Σ -> Ω
          f g σ' = if sem b σ' then g *. sem c σ' else Normal σ'

  sem (If b c1 c2) σ = if sem b σ then sem c1 σ else sem c2 σ
